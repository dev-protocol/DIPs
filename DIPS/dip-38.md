...
| `_calculateCumulativeHoldersRewardAmount` | Change | Add "geometric mean holders reward" to the arguments. Add "cap by geometric mean" to the return value. The "cap by geometric mean" is calculated by multiplying the "geometric mean holders price" by the number of stakings. The number of stakings( value of `getStoragePropertyValue(PROPERTY)` ) uses in the calculation deducts the "disabled lockedups." |
